//*******************************************************************************************
// 文件:	SciEchoback.c
//功能:实现DSP281x SCIA与PC机通信回传实验//
//条件:通过传输电缆将SCIA端口连接到PC机。运用串口调试助手观察来自SCI的数据同时向SCI
//传送信息，通过SCI接收到的字符再回传到主机
// 
//*******************************************************************************************
...
//    msg = "\r\n\n\nHello World!\0";			// 设置邮箱英文信息
    msg = "\r\n\n\n您好!\0";					// 设置邮箱汉字信息

			// "\n"为换行符,"\0" 为字符串结束符，"\r" 为回车符, "\t" 为横向跳格符,相当于Tab键，
			// "\v" 为竖向跳格符，"\b" 为退格符, "\f" 为走纸换页符,
...
//       msg = "\r\nEnter a character: \0";		// 设置邮箱英文信息
       msg = "\r\n请输入一个字符: \0";			// 设置邮箱汉字信息

       scia_msg(msg);							// 发送邮箱信息
       		// 等待一个字符输入
       while(SciaRegs.SCIFFRX.bit.RXFIFST!=1) { } 	
			// 检测SCIFFRX(SCI先进先出接收寄存器)的RXFIFST(接收FIFO状态位，有5位)是否接
			// 收到一个字符(规定RXFIFST最多可接收16个字符)。若为空，等待；若为1，表示接收
			// 到一个字符，则结束等待执行下面的程序。
       ReceivedChar = SciaRegs.SCIRXBUF.all;	// 接收到的一个字符存入ReceivedChar
       		// 回传一个字符

//       msg = "  You sent: \0";				// 设置邮箱英文信息
       msg = "  您发送的字符是: \0";			// 设置邮箱汉字信息

       scia_msg(msg);							// 发送邮箱信息
       scia_xmit(ReceivedChar);					// 仅发送接收到的一个字符
       LoopCount++;								// 循环计数器加1
    }
}
//*******************************************************************************************
